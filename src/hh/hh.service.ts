...
import {HhResponse, TestHhResponse} from "./hh.models";
...
export class HhService {
    ...
    async getData(text: string) {
        try {
            ...
            return this.parseData(testHhData);
        } catch (e) {
            Logger.error(e);
        }
    }

    private parseData(data: TestHhResponse): HhData {
        ...
        const middleSalary = this.getSalaryFromString(salaryCluster.items[Math.ceil(salaryCluster.items.length / 2) - 1].name)
